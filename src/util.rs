...
macro_rules! warn_if {
    ($cond:expr) => {
        if !$cond {
            println!(concat!("WARNING ({}@{}) check failed: ", stringify!($cond)), file!(), line!());
        }
    };
    ($cond:expr, $($arg:tt),+) => {
        if !$cond {
...
macro_rules! debug_warn {
    ($($arg:tt),+) => (if cfg!(debug_assertions) { warn!($($arg),+); })
}

#[macro_export]
macro_rules! debug_warn_if {
    ($cnd:expr, $($arg:tt),*) => (if cfg!(debug_assertions) { warn_if!($cnd, $($arg),*); })
}
