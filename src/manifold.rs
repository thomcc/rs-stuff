...
impl default::Default for Manifold {
    fn default() -> Manifold { Manifold::new() }
}
...
impl<'a> Iterator for FaceViewIterator<'a> {
    ...
    fn next(&mut self) -> Option<&'a HalfEdge> {
        ...
        assert_eq!(self.current, self.mf.edges[self.current as usize].id);
        let result = &self.mf.edges[self.current as usize];
...
impl Manifold {
    ...
    pub fn face_iter<'a>(&'a self, face: usize) -> FaceViewIterator<'a> {
        ...
        FaceViewIterator { mf: self, start: fb, current: fb }
    }
    ...
    pub fn translate(&mut self, t: V3) -> &mut Manifold {
    ...
    pub fn rotate(&mut self, q: Quat) -> &mut Manifold {
        for v in self.verts.iter_mut() {
            let r = q * *v;
            *v = r;
        }
    ...
    pub fn scale3(&mut self, s: V3) -> &mut Manifold {
        for v in self.verts.iter_mut() {
            *v *= s;
        }
        for p in self.faces.iter_mut() {
    ...
    pub fn scale(&mut self, s: f32) -> &mut Manifold {
        self.scale3(V3::splat(s))
    }

    pub fn cropped(&self, slice: Plane) -> Manifold {
        if self.verts.is_empty() || self.faces.is_empty() {
            return self.clone();
        }
        match self.split_test(slice) {
            PlaneTestResult::Over => return Manifold::new(),
    ...
    pub fn dual_r(&self, r: f32) -> Manifold {
        let mut d = Manifold::new();
    ...
    pub fn dual(&self) -> Manifold {
        self.dual_r(1.0)
    }

    pub fn new_box(min: V3, max: V3) -> Manifold {
    ...
    pub fn new_rect(bounds: V3) -> Manifold {
        Manifold::new_box(bounds*-0.5, bounds*0.5)
    }

    pub fn new_cube(r: f32) -> Manifold {
        Manifold::new_box(V3::splat(-r), V3::splat(r))
    }

    pub fn new_mesh(verts: &[V3], tris: &[[u16; 3]]) -> Manifold {
    ...
    pub fn new_cylinder(sides: usize, radius: f32, height: f32) -> Manifold {
        let mut mesh = Manifold::new();
    ...
    pub fn new_cone(sides: usize, radius: f32, height: f32) -> Manifold {
        let mut mesh = Manifold::new();
